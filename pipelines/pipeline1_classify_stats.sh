...
# 
# Uso: bash pipeline1_classify_stats.sh <nombre_proyecto> [config_file]
# Ejemplo: bash pipeline1_classify_stats.sh Proyecto1_20251113
################################################################################
...
# ============================================================================
# PASO 3.5: CLASIFICACIÓN TAXONÓMICA CON SILVA
# ============================================================================

echo ""
echo "=========================================="
echo "PASO 3.5: Clasificación taxonómica"
echo "=========================================="
echo ""

CLASSIFIER="/home/proyecto/tools/silva-138-99-nb-classifier.qza"
OUT_TAX="$QIIME_DIR/taxonomy"
NJOBS=8  # Ajustar según RAM disponible
# con los 12 nucleos totales no funciona por el consumo total de RAM (excede)

if [[ ! -f "$CLASSIFIER" ]]; then
  echo "  ADVERTENCIA: Clasificador no encontrado: $CLASSIFIER"
  echo "  Saltando clasificación taxonómica"
  echo ""
else
  for GRUPO in "${GRUPOS[@]}"; do
    ...
    if [[ ! -f "$REP_SEQS" ]]; then
      echo "  Saltando $GRUPO - no se encontró rep-seqs.qza"
      continue
    fi
    
    # Clasificación con monitoreo
    TAXONOMY_CMD="$CONDA_RUN qiime feature-classifier classify-sklearn \
    ...
    run_monitored "taxonomy_${GRUPO}" "$TAXONOMY_CMD"
    
    # Visualización (sin monitoreo detallado)
    echo "  Generando visualización taxonómica para $GRUPO..."
    $CONDA_RUN qiime metadata tabulate \
      --m-input-file "$OUT_TAXON" \
      --o-visualization "$OUT_TAXON_VIZ"
    
    echo "  ✓ Taxonomía completada para $GRUPO"
  done
  
  echo ""
  echo "✓ Clasificación taxonómica completada para todos los grupos"
  echo ""
fi
...
# Copiar taxonomía
for GRUPO in "${GRUPOS[@]}"; do
  [[ -f "$OUT_TAX/$GRUPO/taxonomy.qzv" ]] && \
    cp "$OUT_TAX/$GRUPO/taxonomy.qzv" "$RESULTS_DIR/taxonomy-${GRUPO}.qzv"
done
