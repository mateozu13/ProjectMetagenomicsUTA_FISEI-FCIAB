...
if [[ ! -f "$CLASSIFIER" ]]; then
  ...
  for GRUPO in "${GRUPOS[@]}"; do
    ...
    if [[ ! -f "$REP_SEQS" ]]; then
      echo "   ADVERTENCIA: No se encontró rep-seqs.qza para $GRUPO"
      continue
    fi
    
    echo "  Ejecutando clasificación (esto puede tomar varios minutos)..."
    $CONDA_RUN qiime feature-classifier classify-sklearn \
    ...
    echo "  ✓ Clasificación completada para $GRUPO"
    echo "    - Taxonomía: $OUT_TAXON"
    echo "    - Visualización: $OUT_TAXON_VIZ"
    echo ""
  done
...
# Copiar taxonomía de cada grupo
for GRUPO in "${GRUPOS[@]}"; do
  if [[ -f "$OUT_TAX/$GRUPO/taxonomy.qzv" ]]; then
